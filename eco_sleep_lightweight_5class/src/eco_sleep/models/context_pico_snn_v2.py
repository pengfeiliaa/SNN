# -*- coding: utf-8 -*-
"""ContextPicoSNNV2: lightweight SNN backbone with optional short-context modeling."""
...
class _MultiScaleStemV2(nn.Module):
    def __init__(
        self,
        in_channels: int,
        branch_channels: int,
...
class ContextPicoSNNV2(nn.Module):
    """Very-light raw stem + token-level spiking encoder + optional short-context SNN."""
    ...
    def __init__(
        ...
        use_n1_aux: bool = True,
        center_residual_weight: float = 0.40,
        ...
        self.multiscale_stem = _MultiScaleStemV2(
        ...
        self.token_head = nn.Linear(self.epoch_embed_dim, self.num_classes)
        self.epoch_readout = nn.Sequential(
            nn.Linear(self.epoch_embed_dim * 2, self.epoch_embed_dim, bias=False),
        ...
        self.context_mix = nn.Sequential(
            nn.Linear(self.epoch_embed_dim + self.context_hidden_size * 2, self.epoch_embed_dim, bias=False),
            nn.LayerNorm(self.epoch_embed_dim),
            nn.SiLU(inplace=True),
            nn.Dropout(self.dropout_p),
        )
        self.context_gate = nn.Linear(self.epoch_embed_dim + self.context_hidden_size * 2, self.epoch_embed_dim)
        ...
        if self.use_aux_heads:
            ...
            self.n1_head = nn.Linear(self.epoch_embed_dim, 1) if self.use_n1_aux else None
        else:
    ...
    def _run_epoch_encoder(self, tokens: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        mean_sum = torch.zeros(batch_size, self.epoch_embed_dim, device=tokens.device, dtype=tokens.dtype)
        max_emb = torch.full_like(mean_sum, fill_value=-1e6)
        token_logits = []
        ...
        for step in range(steps):
            ...
            token_emb = self.epoch_proj(s)
            mean_sum = mean_sum + token_emb
            max_emb = torch.maximum(max_emb, token_emb)
            token_logits.append(self.token_head(token_emb))
            firing_acc = firing_acc + s.mean()

        pooled = torch.cat([mean_sum / float(max(1, steps)), max_emb], dim=-1)
        pooled = self.epoch_readout(pooled)
        firing_rate = firing_acc / float(max(1, steps))
        return pooled, firing_rate, torch.stack(token_logits, dim=1)
    ...
    def forward(self, x: torch.Tensor):
        if x.ndim == 3:
            x = x.unsqueeze(1)
        if x.ndim != 4:
            raise ValueError(f"ContextPicoSNNV2 expects [B,L,C,T] or [B,C,T], got shape={tuple(x.shape)}")
        ...
        epoch_embed, epoch_firing, token_logits = self._run_epoch_encoder(tokens)
        epoch_embed = epoch_embed.view(batch_size, context_len, self.epoch_embed_dim)
        token_logits = token_logits.view(batch_size, context_len, self.t_steps, self.num_classes)

        center_idx = context_len // 2
        center_epoch = epoch_embed[:, center_idx, :]
        center_step_logits = token_logits[:, center_idx, :, :]
        ...
        if context_len > 1:
            ...
            ctx_center_fwd = ctx_fwd[:, center_idx, :]
            ctx_center_bwd = ctx_bwd[:, center_idx, :]
        else:
            ctx_center_fwd = torch.zeros(
                batch_size, self.context_hidden_size, device=x.device, dtype=center_epoch.dtype
            )
            ctx_center_bwd = torch.zeros_like(ctx_center_fwd)
            ctx_firing_fwd = torch.zeros((), device=x.device, dtype=center_epoch.dtype)
            ctx_firing_bwd = torch.zeros((), device=x.device, dtype=center_epoch.dtype)

        context_summary = torch.cat([ctx_center_fwd, ctx_center_bwd], dim=-1)
        gate = torch.sigmoid(self.context_gate(torch.cat([center_epoch, context_summary], dim=-1)))
        mixed_context = self.context_mix(torch.cat([center_epoch, ctx_center_fwd, ctx_center_bwd], dim=-1))
        center_fusion = gate * mixed_context + (1.0 - gate) * center_epoch
        center_fusion = center_fusion + float(self.center_residual_weight) * center_epoch
        ...
        outputs = {
            ...
            "step_logits": center_step_logits,
        ...
        if self.use_aux_heads and self.sleep_wake_head is not None and self.rem_head is not None:
            ...
            if self.n1_head is not None:
                outputs["n1"] = self.n1_head(center_fusion).squeeze(-1)
        return outputs
